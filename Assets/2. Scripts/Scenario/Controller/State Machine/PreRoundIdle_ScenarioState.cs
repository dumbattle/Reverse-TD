...
namespace Core {
    public class PreRoundIdle_ScenarioState : IFSM_State<ScenarioInstance> {
        ...
        public IFSM_State<ScenarioInstance> Update(ScenarioInstance s) {
            ...
            if (InputManager.TileSelect.yes) {
                if (s.parameters.creepPathfinder.GetStartIndicies().Contains(InputManager.TileSelect.index)) {
                    return PreRoundIdle_CreepMenuState.Get(s);
                }
            }
            ...
            //}
            //if (creepMenu.details.buttons.infoSubmenuButton.Clicked) {
            //    creepMenu.OpenInfoSubmenu();
            //}
